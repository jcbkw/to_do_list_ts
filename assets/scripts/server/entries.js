...
var api = {
    GET: function (request, response, storage) {
        storage.read(function (data) {
            ...
            // get list
            var result = [], contains = get('contains', null);
            ...
            switch (get('sort', null)) {
                case 'asc':
                    result.sort(function (a, b) { return a.lastModified - b.lastModified; });
                    break;
                case 'desc':
                    result.sort(function (a, b) { return b.lastModified - a.lastModified; });
                    break;
            }
            // (optional) limit and from
            var limit = getNonNegativeInteger('limit', Number.NaN), from = getNonNegativeInteger('from', Number.NaN), hasLimit = !isNaN(limit), hasFrom = !isNaN(from);
            if (hasLimit || hasFrom) {
    ...
    PATCH: function (request, response, storage) {
        storage.read(function (data) {
            var id = get('id');
            if (data.hasOwnProperty(id)) {
                var entry = data[id], hasChanged = false, message = get('message', null), status = getNonNegativeInteger('status', Number.NaN);
                if (message !== null) {
...
module.exports = function (request, response) {
    ...
    call(response, api[request.method], request, response, storage(require('path').join(__dirname, 'data.json'), response));
};
var get;
